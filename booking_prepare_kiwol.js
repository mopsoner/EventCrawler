...
async function selectGender(page) {
  const candidates = ["select[name*='gender']", "select[name*='civil']", "select[name*='sexe']", "select[name*='title']", "select[name*='civilite']", "select[name*='civility']"];
  for (const selector of candidates) {
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (await item.isVisible()) {
          for (const label of [DEFAULT_GENDER, 'Homme', 'Male', 'Mr', 'Monsieur', 'M.']) {
...
async function fillFormByLabels(page, email) {
  ...
  const groups = [
    [DEFAULT_FIRST_NAME, ["input[name*='firstname']","input[name*='first_name']","input[id*='firstname']","input[id*='first_name']","input[name*='prenom']","input[id*='prenom']"]],
    [DEFAULT_LAST_NAME, ["input[name*='lastname']","input[name*='last_name']","input[id*='lastname']","input[id*='last_name']","input[name*='nom']","input[id*='nom']"]],
    ...
    [DEFAULT_PHONE, ["input[name*='phone']","input[name*='mobile']","input[name*='tel']","input[id*='phone']","input[id*='mobile']","input[name*='telephone']"]],
  ];
...
async function detectSuccess(page) {
  ...
  const selectors = selectorsFor('success', [
    "h1:has-text('Confirmé')",
    "h1:has-text('Confirmed')",
    "h2:has-text('Confirmé')",
...
async function runPrepare(eventUrl, ticketCount, email, productName) {
  ...
  try {
    ...
    await page.goto(eventUrl, { timeout: 60000, waitUntil: 'domcontentloaded' });
    try { await page.waitForLoadState('networkidle', { timeout: 15000 }); } catch {}

    const pageTitle = await page.title().catch(() => '');
    logLine(`[kiwol] Page loaded: title="${pageTitle}" url=${page.url()}`);

    const bodyText = await page.locator('body').innerText().catch(() => '');
    if (bodyText.includes('Just a moment') || bodyText.includes('Checking your browser')) {
      logLine('[kiwol] Cloudflare challenge detected, waiting for bypass...');
      await page.waitForTimeout(8000);
      try { await page.waitForLoadState('networkidle', { timeout: 20000 }); } catch {}
    }

    await screenshot(page, `${prefix}-01-loaded`);
    ...
    const reserveSelectors = selectorsFor('reserve', [
    ...
    lastStepName = 'click_reserve'; lastIntent = 'reserve'; lastSelectors = reserveSelectors;
    const reserved = await clickFirstVisible(page, reserveSelectors, 10000);
    if (!reserved) {
      logLine(`[kiwol] WARNING: No reserve button found (tried ${reserveSelectors.length} selectors). Proceeding directly to checkout step.`);
    } else {
      logLine('[kiwol] Reserve/Commander button clicked successfully.');
    }
    await page.waitForTimeout(2000);
    try { await page.waitForLoadState('networkidle', { timeout: 15000 }); } catch {}
    await screenshot(page, `${prefix}-03-after-reserve`);

    const checkoutSelectors = selectorsFor('checkout', [
      "button:has-text('Continuer')",
      "button:has-text('Passer au paiement')",
      "button:has-text('Procéder au paiement')",
      "button:has-text('Continue')",
      "button:has-text('Proceed to checkout')",
      "a:has-text('Passer au paiement')",
    ]);
    lastStepName = 'proceed_checkout'; lastIntent = 'checkout'; lastSelectors = checkoutSelectors;
    const proceeded = await clickFirstVisible(page, checkoutSelectors, 6000);
    if (!proceeded) {
      logLine(`[kiwol] WARNING: No checkout continuation button found (tried ${checkoutSelectors.length} selectors). Entering step loop directly.`);
    } else {
      logLine('[kiwol] Checkout continuation button clicked successfully.');
    }
    await page.waitForTimeout(2000);
    try { await page.waitForLoadState('networkidle', { timeout: 15000 }); } catch {}

    for (let step = 1; step <= 8; step++) {
      await screenshot(page, `${prefix}-step-${step}`);
      ...
      if (successBefore) {
        ...
        logLine(`[kiwol] Success detected before advance on step ${step}: ${successBefore}`);
        return;
      }
      const advanceSelectors = selectorsFor('advance', [
      ...
      if (!advanced) {
        logLine(`[kiwol] No advance button found on step ${step}, stopping.`);
        break;
      }
      ...
      if (successAfter) {
        ...
        logLine(`[kiwol] Success detected after advance on step ${step}: ${successAfter}`);
        return;
      }
    }
    ...
    logLine(`[kiwol] Flow failed at step '${lastStepName}': ${err}`);
    throw err;
  } finally {
    await browser.close();
  }
...
(async () => {
  try {
    ...
    if (!args['event-url'] || !args['ticket-count'] || !args['email'] || !args['product-name']) {
      console.error('Usage: node booking_prepare_kiwol.js --event-url <url> --ticket-count <n> --email <email> --product-name <name>');
      process.exit(1);
    }
